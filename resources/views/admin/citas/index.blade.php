...
@section('content')
    <div class="max-w-6xl mx-auto py-8">
        <h2 class="text-3xl font-extrabold mb-6 text-[#1db954] flex items-center gap-2">
            <i class="fas fa-calendar-alt text-[#1db954]"></i> Listado de Citas
        </h2>
        ...
        <div class="overflow-x-auto rounded-xl border border-[#1db954]/20 bg-[#181818] animate-fade-in">
            <table id="tablaAdminCitas" class="min-w-full text-sm text-left text-white">
                ...
                <tbody class="divide-y divide-[#1db954]/10">
                        ...
                        <tr class="hover:bg-[#1db954]/10 transition">
                            ...
                            <td class="px-4 py-2 flex gap-2">
                                ...
                                @if ($cita->estado !== 'completada')
                                    <form action="{{ route('admin.citas.completar', $cita->id) }}" method="POST"
                                        onsubmit="return confirm('¿Marcar como completada?');" class="inline">
                                        @csrf
                                        @method('PUT')
                                        <button type="submit" class="text-green-400 hover:text-[#1db954] transition"
                                            title="Completar"><i class="fas fa-check"></i></button>
                                    </form>
                                @endif
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-4 py-6 text-center text-white/60">No hay citas registradas.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <script>
        function filtrarAdminCitas() {
            const input = document.getElementById('searchAdminCitas');
            const filter = input.value.toLowerCase();
            const table = document.getElementById('tablaAdminCitas');
