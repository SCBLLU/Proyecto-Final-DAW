...
<div class="max-w-lg mx-auto py-8">
    ...
    <form action="{{ route('admin.citas.update', $cita->id) }}" method="POST" class="space-y-4 animate-fade-in">
        ...
        <div>
            ...
            <input type="date" name="fecha" id="fecha" required min="{{ now()->toDateString() }}" value="{{ $fechaSeleccionada }}" class="w-full bg-[#181818] border border-[#1db954]/30 text-white rounded px-3 py-2 focus:border-[#1db954] focus:ring-[#1db954]" />
        </div>
        <div>
            <label class="block text-white/80 mb-1">Cliente:</label>
            <select name="usuario_id" required class="w-full bg-[#181818] border border-[#1db954]/30 text-white rounded px-3 py-2">
        ...
        <div>
            ...
            <select name="hora" required class="w-full bg-[#181818] border border-[#1db954]/30 text-white rounded px-3 py-2">
        ...
        <div>
            ...
            <select name="corte_cabello" required class="w-full bg-[#181818] border border-[#1db954]/30 text-white rounded px-3 py-2">
        ...
        <div>
            ...
            <select name="estado" required class="w-full bg-[#181818] border border-[#1db954]/30 text-white rounded px-3 py-2">
        ...
        <button type="submit" class="w-full px-5 py-2 bg-[#1db954] text-black rounded-full font-bold border border-[#1db954] transition-all duration-300 shadow-md group hover:bg-[#121212] hover:text-[#1db954] hover:border-[#1db954] focus:outline-none focus:ring-2 focus:ring-[#1db954] focus:ring-offset-2">
            <i class="fas fa-save transition-transform duration-300 group-hover:rotate-180 group-hover:text-[#1db954]"></i> Actualizar Cita
        </button>
    </form>
</div>
<style>
